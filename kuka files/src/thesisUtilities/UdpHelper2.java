...
public class UdpHelper2 extends Thread
{
    ...
    // Carol was here 
    public double[] qA17, qA17d, tA17, fEE, pEE;
    // gains and energy stuff
    public double stiffnessAxis = 0, stiffnessAngles = 0, damping = 0;
    public double velJS = 0, force = 0.0;
    // trajectory stuff
    public double[] sim_qA;
    boolean sim_qA_error;
    public int status = 0;
    /**
     * Constructor.
     * 
     ...
     */
    public UdpHelper2(int localPort, String packetIdentifier, ITaskLogger logger)
    {
    ...
    private String[] handleMessage(DatagramPacket receivedPacket)
    {
        ...
        else if (message[1].equals("GAINS")) {
        	sendMatlabVariablesBack();
        	
        	return message;       	
       }
        else if (message[1].equals("QA17")) {
        	
        	if(message.length > 8 && message.length < 11)
        	{
            	// convert to double
            	for(int k=0;k<7;k++)
            	{
            		sim_qA[k] = Double.parseDouble(message[k+2]);
            	}        		
        	}
        	else
        	{
        ...
        else if (message[1].equals("MATLAB")) {
        	sendMatlabVariablesBack();
   ...
   //
    private void sendMatlabVariablesBack(){
    	// Carol was here messing around
    	String Cmsg = new String(stiffnessAxis+";"+stiffnessAngles+";"+damping+";");
    	String Vmsg = new String(velJS+";"+force+";");
    	String fullMsgString = new String(status + ";" + Cmsg + Vmsg);
        ...
        {
            _logger.error("Failed sending UDP STATUS variables to MATLAB: ", e);
        }
    }
...
//
    private void sendStatusVariablesBack(){
    	...
    	String tMsg = new String(tA17[0]+";"+tA17[1]+";"+tA17[2]+";"+tA17[3]+";"+tA17[4]+";"+tA17[5]+";"+tA17[6]+";");
    	String pMsg = new String(pEE[0]+";"+pEE[1]+";"+pEE[2]+";"+pEE[3]+";"+pEE[4]+";"+pEE[5]+";");
    	String fMsg = new String(fEE[0]+";"+fEE[1]+";"+fEE[2]+";"+fEE[3]+";"+fEE[4]+";"+fEE[5]+";");

    	String fullMsgString = new String(qMsg+tMsg+pMsg+fMsg);
    	//_logger.info("Sending: " + fullMsgString);
        try
        {
            byte[] msg = fullMsgString.getBytes(UTF_8);
    ...
    private void sendQVariablesBack(){
    	// Carol was here messing around
    	String qMsg = new String(qA17[0]+";"+qA17[1]+";"+qA17[2]+";"+qA17[3]+";"+qA17[4]+";"+qA17[5]+";"+qA17[6]+";");

    	String fullMsgString = new String("ROBERT says hi");
    	//_logger.info("Sending: " + fullMsgString);
        try
        {
            byte[] msg = fullMsgString.getBytes(UTF_8);
